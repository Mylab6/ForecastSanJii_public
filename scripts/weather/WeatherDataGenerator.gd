...
static func generate_scattered_storms(center: Vector2, count: int = 5, base_intensity: float = 0.4, 
	...
	for i in range(count):
		...
		var intensity = base_intensity + randf_range(-0.2, 0.3)
		intensity = clamp(intensity, 0.1, 1.0)
		
		var size = randf_range(0.8, 2.0)
		var lifetime = randf_range(8.0, 15.0)
		
		# Random movement (typical storm motion)
		var motion_angle = randf_range(-PI/4, PI/4) + PI/4  # Generally northeast
		var motion_speed = randf_range(0.005, 0.015)  # Slow movement
...
static func generate_supercell(center: Vector2, intensity: float = 0.8) -> Array[WeatherEcho]:
	...
	for i in range(6):
		...
		var meso_intensity = intensity - 0.3 + randf_range(-0.1, 0.1)
...
static func generate_hurricane(center: Vector2, intensity: float = 0.9, eye_radius: float = 0.08) -> Array[WeatherEcho]:
	...
	for i in range(eyewall_segments):
		...
		var wall_intensity = intensity + randf_range(-0.1, 0.05)
...
static func generate_squall_line(start_pos: Vector2, end_pos: Vector2, intensity: float = 0.6) -> Array[WeatherEcho]:
	...
	for i in range(segments):
		...
		var line_intensity = intensity + randf_range(-0.15, 0.2)
		...
		if randf() < 0.3 and i > 2 and i < segments - 3:
...
static func generate_frontal_system(center: Vector2, width: float = 0.4, intensity: float = 0.5) -> Array[WeatherEcho]:
	...
	for row in range(rows):
		for col in range(cols):
			...
			var front_intensity = intensity + randf_range(-0.2, 0.2)
			front_intensity = clamp(front_intensity, 0.1, 0.8)
			
			# Frontal systems now stationary
			var front_velocity = Vector2.ZERO
			
			var size = randf_range(1.0, 1.8)
			var lifetime = randf_range(12.0, 20.0)
...
static func generate_random_weather_scenario(radar_bounds: Rect2) -> Array[WeatherEcho]:
	...
	match scenario:
		"scattered":
			return generate_scattered_storms(center, randi_range(3, 8), randf_range(0.3, 0.7))
		"supercell":
			return generate_supercell(center, randf_range(0.7, 0.9))
		"hurricane":
			return generate_hurricane(center, randf_range(0.8, 1.0), randf_range(0.06, 0.12))
		"squall_line":
			var end_pos = center + Vector2(randf_range(-0.3, 0.3), randf_range(-0.2, 0.2))
			return generate_squall_line(center, end_pos, randf_range(0.5, 0.8))
		"frontal":
			return generate_frontal_system(center, randf_range(0.3, 0.5), randf_range(0.4, 0.6))
		"mixed":
			var all_echoes: Array[WeatherEcho] = []
			all_echoes.append_array(generate_scattered_storms(center, 3, 0.4))
			if randf() < 0.6:
				var second_center = Vector2(randf_range(0.2, 0.8), randf_range(0.2, 0.8))
...
static func get_city_weather_echoes() -> Array[WeatherEcho]:
	...
	var num_affected = randi_range(2, 4)  # 2-4 cities with weather
	var affected_cities: Array[String] = []
	
	# Randomly select cities to have weather
	for i in range(num_affected):
		var random_city = all_cities[randi() % all_cities.size()]
	...
	for city_name in affected_cities:
		...
		var base_intensity = 0.5 + randf_range(-0.1, 0.2)  # Random variation
		
		var city_echoes = generate_scattered_storms(city_pos, randi_range(2, 5), base_intensity, 0.08)
